...
function enviarMensaje() {
  ...
  mensajes.innerHTML += '<div style="background:#1A1A1A;padding:.8rem 1rem;font-size:.85rem;color:#F5F5F0;border-left:2px solid #F5F5F0;margin-left:1rem;">' + mensaje + '</div>';
  input.value = '';
  mensajes.scrollTop = mensajes.scrollHeight;
  const loadId = 'load' + Date.now();
  mensajes.innerHTML += '<div id="' + loadId + '" style="background:#1A1A1A;padding:.8rem 1rem;font-size:.85rem;color:#666;">Escribiendo...</div>';
  mensajes.scrollTop = mensajes.scrollHeight;
  fetch('/chatbot', {
    method: 'POST',
  ...
  .then(data => {
    ...
    const texto = data.respuesta.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>').replace(/\n/g, '<br>');
    mensajes.innerHTML += '<div style="background:#1A1A1A;padding:.8rem 1rem;font-size:.85rem;color:#CCCCCC;line-height:1.6;">' + texto + '</div>';
    mensajes.scrollTop = mensajes.scrollHeight;
  })
  .catch(() => {
    document.getElementById(loadId).remove();
    mensajes.innerHTML += '<div style="background:#1A1A1A;padding:.8rem 1rem;font-size:.85rem;color:#C0392B;">Error al conectar.</div>';
  });
}
</script>
<div id="chat-btn" onclick="toggleChat()"
     style="position:fixed;bottom:2rem;right:2rem;width:52px;height:52px;
            background:#F5F5F0;border-radius:50%;cursor:pointer;
            display:flex;align-items:center;justify-content:center;
            box-shadow:0 4px 20px rgba(0,0,0,0.4);z-index:9999;
            font-size:.7rem;font-weight:900;color:#0A0A0A;letter-spacing:1px;">AI</div>
<div id="chat-box"
     style="display:none;flex-direction:column;position:fixed;bottom:5.5rem;right:2rem;
            width:360px;background:#111111;border:1px solid #2A2A2A;
            z-index:9998;max-height:500px;">
  <div style="padding:1rem 1.2rem;border-bottom:1px solid #2A2A2A;
              display:flex;justify-content:space-between;align-items:center;">
    <span style="font-size:.65rem;font-weight:700;letter-spacing:3px;
                 text-transform:uppercase;color:#F5F5F0;">Asistente IA</span>
    <span onclick="toggleChat()" style="cursor:pointer;color:#666;font-size:1.4rem;line-height:1;">&times;</span>
  </div>
  <div id="chat-mensajes"
       style="overflow-y:auto;padding:1rem;display:flex;flex-direction:column;
              gap:.8rem;min-height:200px;max-height:320px;">
    <div style="background:#1A1A1A;padding:.8rem 1rem;font-size:.85rem;color:#CCCCCC;line-height:1.6;">
      Hola. Soy el asistente de AetherCode. Puedo ayudarte con Python, JavaScript, Java, C++ y SQL.
    </div>
  </div>
  <div style="border-top:1px solid #2A2A2A;display:flex;">
    <input id="chat-input" type="text" placeholder="Escribe tu pregunta..."
           onkeydown="if(event.key==='Enter') enviarMensaje()"
           style="flex:1;background:transparent;border:none;padding:.8rem 1rem;
                  color:#F5F5F0;font-family:inherit;font-size:.85rem;outline:none;">
    <button onclick="enviarMensaje()"
            style="background:#F5F5F0;color:#0A0A0A;border:none;padding:.8rem 1.2rem;
                   cursor:pointer;font-weight:700;font-size:.7rem;letter-spacing:1px;">
      ENVIAR
    </button>
  </div>
</div>
...
var C = {{
  toggle: function() {{
    ...
    if (b.style.display === 'flex') document.getElementById('chat-input').focus();
  }},
  enviar: function() {{
    var inp = document.getElementById('chat-input');
    var msg = inp.value.trim();
    if (!msg) return;
    var box = document.getElementById('chat-mensajes');
    box.innerHTML += '<div style="background:#1A1A1A;padding:.8rem;font-size:.85rem;color:#F5F5F0;border-left:2px solid #F5F5F0;margin-left:1rem;">' + msg + '</div>';
    inp.value = '';
    var lid = 'l' + Date.now();
    box.innerHTML += '<div id="' + lid + '" style="background:#1A1A1A;padding:.8rem;font-size:.85rem;color:#666;">Escribiendo...</div>';
    box.scrollTop = box.scrollHeight;
    fetch('/chatbot', {{method:'POST',headers:{{'Content-Type':'application/x-www-form-urlencoded'}},body:'mensaje='+encodeURIComponent(msg)}})
    .then(function(r) {{ return r.json(); }})
    .then(function(d) {{
      var el = document.getElementById(lid);
      if (el) el.remove();
      box.innerHTML += '<div style="background:#1A1A1A;padding:.8rem;font-size:.85rem;color:#CCCCCC;line-height:1.6;">' + d.respuesta.replace(/\\n/g,'<br>') + '</div>';
      box.scrollTop = box.scrollHeight;
    }})
    .catch(function() {{
      var el = document.getElementById(lid);
      if (el) el.remove();
      box.innerHTML += '<div style="background:#1A1A1A;padding:.8rem;font-size:.85rem;color:#C0392B;">Error al conectar.</div>';
    }});
  }}
}};
</script>
...
<div id="chat-box"
     style="display:none;flex-direction:column;position:fixed;bottom:5.5rem;right:2rem;
            width:360px;background:#111;border:1px solid #2A2A2A;z-index:9998;max-height:500px;">
  <div style="padding:1rem;border-bottom:1px solid #2A2A2A;display:flex;justify-content:space-between;align-items:center;">
    <span style="font-size:.65rem;font-weight:700;letter-spacing:3px;color:#F5F5F0;">ASISTENTE IA</span>
    <span onclick="C.toggle()" style="cursor:pointer;color:#666;font-size:1.4rem;">&times;</span>
  </div>
  <div id="chat-mensajes"
       style="overflow-y:auto;padding:1rem;display:flex;flex-direction:column;gap:.8rem;min-height:200px;max-height:320px;">
    <div style="background:#1A1A1A;padding:.8rem;font-size:.85rem;color:#CCCCCC;line-height:1.6;">
      Hola. Soy el asistente de AetherCode. Puedo ayudarte con Python, JavaScript, Java, C++ y SQL.
    </div>
  </div>
  <div style="border-top:1px solid #2A2A2A;display:flex;">
    <input id="chat-input" type="text" placeholder="Escribe tu pregunta..."
           onkeydown="if(event.key==='Enter') C.enviar()"
           style="flex:1;background:transparent;border:none;padding:.8rem;color:#F5F5F0;font-family:inherit;font-size:.85rem;outline:none;">
    <button onclick="C.enviar()"
            style="background:#F5F5F0;color:#0A0A0A;border:none;padding:.8rem 1.2rem;cursor:pointer;font-weight:700;font-size:.7rem;letter-spacing:1px;">
      ENVIAR
    </button>
  </div>
</div>
